...
/** \file tests/solver/solverrequester_test.cc
 *
 * Ported from zypper's tests/SolverRequester_test.cc — every assertion,
 * fixture, and expected value below is unchanged from upstream. Only the
 * plumbing that talks to the API was touched: zypper's SolverRequester is
 * now solverequest::Requester (see worker/src/solver/), zypper's TestSetup
 * is now the zypper-independent solverequest_test::SolverTestSetup (see
 * testsetup.h), and the handful of sr.install(...)/remove(...)/update(...)
 * convenience-method calls now go through the free functions in
 * solverrequester_helpers.h instead — Requester itself takes an explicit
 * ResPool per submit() rather than reaching for a global one.
 *
 * Checks whether the requester issued correct zypp/solver requests given
...
#include "testsetup.h"
#include "solverrequester_helpers.h"

using namespace zypp;
using namespace solverequest_test; // install()/remove()/update() free functions
...
struct TestInit {
  TestInit()
    : testSetup( std::make_unique<solverequest_test::SolverTestSetup>( Arch_x86_64 ) )
  {
    MIL << "============setup===========" << endl;
  ...
  std::unique_ptr<solverequest_test::SolverTestSetup> testSetup;
};
...
BOOST_AUTO_TEST_CASE(install1)
{
  ...
  solverequest::Requester sr;

  install(sr, rawargs);

  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME_TRYING_CAPS));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_FOUND_CAP));
}
...
BOOST_AUTO_TEST_CASE(install2)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::SET_TO_INSTALL));
...
BOOST_AUTO_TEST_CASE(install4)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::ALREADY_INSTALLED));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NO_UPD_CANDIDATE));
}
...
BOOST_AUTO_TEST_CASE(install6)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME_TRYING_CAPS));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::ADDED_REQUIREMENT));
...
BOOST_AUTO_TEST_CASE(install7)
{
  ...
  std::vector<std::string> rawargs;
  rawargs.push_back("perl(Net::SSL)");
  solverequest::Options sropts;
  sropts.setForceByName();
  solverequest::Requester sr(sropts);

  install(sr, rawargs);

  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME));
  BOOST_CHECK(!sr.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME_TRYING_CAPS));
}
...
BOOST_AUTO_TEST_CASE(install9)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::UPD_CANDIDATE_CHANGES_VENDOR));
}
...
BOOST_AUTO_TEST_CASE(install10)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::ALREADY_INSTALLED));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_IN_REPOS));
}
...
BOOST_AUTO_TEST_CASE(install100)
{
  ...
  BOOST_CHECK(!sr.hasFeedback(solverequest::Feedback::SET_TO_INSTALL));
  BOOST_CHECK(sr.toInstall().empty());
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::UPD_CANDIDATE_IS_LOCKED));
...
BOOST_AUTO_TEST_CASE(install200)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::UPD_CANDIDATE_HAS_LOWER_PRIO));
}
...
BOOST_AUTO_TEST_CASE(install301)
{
  ...
  BOOST_CHECK(!sr.hasFeedback(solverequest::Feedback::SET_TO_INSTALL));
  BOOST_CHECK(sr.toInstall().empty());
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::PATCH_NOT_NEEDED));
}
...
BOOST_AUTO_TEST_CASE(install400)
{
  ...
  BOOST_CHECK(!sr.hasFeedback(solverequest::Feedback::ALREADY_INSTALLED));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::FORCED_INSTALL));
  BOOST_CHECK_EQUAL(sr.toInstall().size(), 1);
  BOOST_CHECK(hasPoolItem(sr.toInstall(), "cron", Edition("4.1-195.0"), Arch_x86_64));
  BOOST_CHECK(!sr.hasFeedback(solverequest::Feedback::UPD_CANDIDATE_HAS_LOWER_PRIO));
}
...
{
  MIL << "<===========remove1================>" << endl;
  solverequest::PackageArgs::Options argopts;
  argopts.doByDefault = false;

  std::vector<std::string> rawargs;
  rawargs.push_back("nonsense");
  solverequest::PackageArgs args(rawargs, ResKind::package, argopts);
  solverequest::Requester sr;
...
BOOST_AUTO_TEST_CASE(remove2)
{
  ...
  solverequest::Options sropts;
  sropts.setForceByName();
  solverequest::Requester sr1(sropts);

  remove(sr1, args);
  BOOST_CHECK(sr1.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME));
  BOOST_CHECK(!sr1.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME_TRYING_CAPS));
  BOOST_CHECK(!sr1.hasFeedback(solverequest::Feedback::NOT_FOUND_CAP));
}
...
BOOST_AUTO_TEST_CASE(remove3)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_INSTALLED));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NOT_FOUND_NAME_TRYING_CAPS));
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::NO_INSTALLED_PROVIDER));
}

// request : remove mc
// response: not installed, fall back to caps, no provider installed
// this one is done by sr.remove(std::vector<std::string>) instead of solverequest::PackageArgs
BOOST_AUTO_TEST_CASE(remove4)
{
  MIL << "<==============remove4=============>" << endl;
  // beware of implicit conversion from std::vector<std::string> to solverequest::PackageArgs
  // if not avoided, the resulting solverequest::PackageArgs would have
  // solverequest::PackageArgs::Options::doByDefault == true! => args without +/- modifiers
...
BOOST_AUTO_TEST_CASE(remove6)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::SET_TO_REMOVE));
...
BOOST_AUTO_TEST_CASE(remove7)
{
  ...
  BOOST_CHECK(sr.hasFeedback(solverequest::Feedback::ADDED_CONFLICT));
...
BOOST_AUTO_TEST_CASE(update1)
{
  ...
  std::vector<std::string> rawargs;
  rawargs.push_back("vim");
  solverequest::PackageArgs args(rawargs);
  solverequest::Requester sr;
